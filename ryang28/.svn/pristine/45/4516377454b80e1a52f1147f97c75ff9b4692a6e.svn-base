...
def toy_fn_1(arg1, arg2):
    ...
    first = arg1*arg1*arg1
    second = tf.scalar_mul(4,(arg2*arg2))
    third = tf.scalar_mul(10, arg1)
...
def toy_fn_2(arg1, arg2):
    ...
    # Input your code here
    sess = tf.Session()
    sum_over = tf.reduce_sum(arg1, 1)
    #print(sess.run(sum_over))
    m,n = arg1.get_shape()
    maximum = tf.reduce_max(arg2)
    max_vector = tf.fill([1,m],maximum)
    #print(sess.run(max_vector))
    return sum_over - max_vector
...
def toy_fn_3(arg1, arg2):
    ...
    #print(sess.run(tf.reshape(new,[-1])))
    return tf.reshape(new, [-1])
