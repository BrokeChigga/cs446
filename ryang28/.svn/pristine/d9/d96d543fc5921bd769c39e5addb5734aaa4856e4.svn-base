...
class LinearMRF(object):
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        f_unary = tf.Variable(unary_features, dtype = tf.float32)
        #f_unary = tf.cast(f_unary, dtype = np.float64)
        phi = tf.multiply(self.unary_weight, f_unary)
        return phi
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        f_pairwise = tf.Variable(pairwise_features, dtype = tf.float32)
        phi = tf.multiply(self.pairwise_weight, f_pairwise)
        return phi
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        #######################
        '''
        #f function for k_th image
        # unary_potentials = self.calculate_unary_potentials(img_features)    #or just img_features?
        # pairwise_potentials = tf.multiply(self.pairwise_weight, self.get_pairwise_beliefs(img_features))
        # f_function = tf.cast(tf.add(tf.reduce_sum(unary_potentials), tf.reduce_sum(pairwise_potentials)), dtype=tf.float32)

        for k in range(len(unary_beliefs)):
            bk = unary_beliefs[k]   #unary_belief for k_th image, shape (width x height, 2)
            uk = unary_potentials[k]    #unary_potential for k_th image, shape (width x height, 2)

            #unary_sum for k_th image, stored in sum_k
            sum_k = tf.cast(tf.reduce_sum(tf.multiply(bk, uk)), dtype=tf.float32)

            #pairwise_sum for k_th image, stored in sum_pair
            sum_pair = tf.cast(tf.reduce_sum(tf.multiply(pair_beliefs, pairwise_potentials)), dtype=tf.float32)

            unary_potentials = self.calculate_unary_potentials(img_features)    #or just img_features?
            pairwise_potentials = tf.multiply(self.pairwise_weight, self.get_pairwise_beliefs(img_features))
            f_function = tf.cast(tf.add(tf.reduce_sum(unary_potentials), tf.reduce_sum(pairwise_potentials)), dtype=tf.float32)
            #f_function = tf.reduce_sum(tf.multiply(self.unary_weight, img_features))

            obj = tf.add(obj, tf.subtract(tf.add(sum_k, sum_pair), f_function))
        '''
        for i in range(len(unary_beliefs)):
            ...
            F_score = tf.reduce_sum(tf.multiply(self.unary_weight, img_features)) + tf.reduce_sum(tf.multiply(self.pairwise_weight,self.get_pairwise_beliefs(img_features)))
            obj = tf.add(obj, tf.cast(unary_part, tf.float32) + tf.cast(pair_part, tf.float32) - F_score)
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        w_h = unary_beliefs.shape[0]
        sequence = np.random.permutation(w_h)
        for idx in sequence:
            score_0 = self.calculate_local_score(idx, 0, unary_beliefs, unary_pots, pairwise_pots)
            score_1 = self.calculate_local_score(idx, 1, unary_beliefs, unary_pots, pairwise_pots)
            if score_0 > score_1:
                unary_beliefs[idx][0] = 1
                unary_beliefs[idx][1] = 0
            else:
                unary_beliefs[idx][0] = 0
                unary_beliefs[idx][1] = 1
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        for n in self.neighbors[node]:
            idx = self.pair_inds[(node, n)]
            belief = np.argmax(unary_beliefs[n])
            score += pairwise_potentials[idx, 2*assignment+belief]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #######################
        count = 0
        for i in range(len(new_unary_beliefs)):
            if new_unary_beliefs[i,0] != old_unary_beliefs[i,0]:
                count += 1
        percentage = 1.0*count/len(new_unary_beliefs) 
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        for node in range(len(unary_beliefs)):
            for neighbor in self.neighbors[node]:
                y_m = np.argmax(unary_beliefs[node])
                y_n = np.argmax(unary_beliefs[neighbor])
                #print(2*y_m+y_n)
                idx = 2*y_m+y_n
                row_idx = self.pair_inds[(node, neighbor)]
                result[row_idx][idx] = 1
        return result
