...
def get_action_index(readout_t, epsilon, t):
    ...
    action_index = 0
    if t < OBSERVE:
        action_index = random.randrange(len(readout_t))
    else:
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    img_t, r_t, terminal = game_state.frame_step(a_t)
    #-----------------------------------------------------------------
    x_t = cv2.cvtColor(cv2.resize(img_t, (80, 80)), cv2.COLOR_BGR2GRAY)
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i] == 1:
            target_q_batch.append(r_batch[i])
        else:
            #copy from piazza
            q_val = alpha*(r_batch[i] + GAMMA*np.max(readout_j1_batch[i]))
