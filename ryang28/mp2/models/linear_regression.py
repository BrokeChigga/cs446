...
class LinearRegression(LinearModel):
    ...
    def backward(self, f, y):
        ...
        Returns:
            total_grad(numpy.ndarray): Gradient of L w.r.t to self.w,
              dimension (ndims+1,1).
        """
        total_grad = np.matmul(self.x.T, (np.matmul(self.x, self.w)-y)) + self.w_decay_factor*self.w
    ...
    def total_loss(self, f, y):
        ...
        """
        total_loss = None
        square_loss = 0.5*np.power(np.linalg.norm(f-y), 2)
        #reg???
        reg = 0.5*self.w_decay_factor*np.power(np.linalg.norm(self.w), 2)
        total_loss = square_loss + reg
        return total_loss
